...
export const Skiper58: React.FC<{ className?: string }> = ({ className }) => (
  <ul
    className={cn(
      "flex min-h-full w-full flex-1 flex-col items-center justify-center gap-4 sm:gap-6 rounded-2xl px-7 py-3 backdrop-blur-sm",
      className
    )}
  >
    {navigationItems.map((item, index) => (
      <li className="relative flex flex-col items-center" key={index}>
        ...
        >
          <TextRoll
            center
            className="text-5xl sm:text-6xl md:text-7xl font-extrabold uppercase leading-[0.8] tracking-[-0.03em] transition-colors lg:text-8xl"
          >
            {item.name}
          </TextRoll>
